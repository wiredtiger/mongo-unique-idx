...
void KeyString::resetToKey(const BSONObj& obj, Ordering ord, RecordId recordId) {
    ...
    dassert(getBuffer()[getSize() - 1] != kEnd);
}
...
void KeyString::appendRecordId(RecordId loc) {
    ...
    // As a special consideration, we never want the last byte to be 0x04, so we can tell a
    // KeyString with appended RecordId apart from one without.
    int extraBytesNeeded =
        bitsNeeded <= 10 ? 0 : ((bitsNeeded - 10) + 7) / 8;  // ceil((bitsNeeded - 10) / 8)
    if (extraBytesNeeded == 4)
        extraBytesNeeded++;
