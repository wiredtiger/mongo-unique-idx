...
Status ReplicationCoordinatorExternalStateImpl::initializeReplSetStorage(OperationContext* opCtx,
    ...
    try {
        ...
        // Updates unique index format version for all non-replicated collections. It is possible
        ...
        // because ShardServers are started up by default with featureCompatibilityVersion 3.6, so
        // we don't want update those indexes until the cluster's featureCompatibilityVersion is
        // explicitly to 4.0 by the config server. The below unique index update for non-replicated
